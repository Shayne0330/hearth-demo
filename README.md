# Hearth

Hearth 是一个面向多项目、多 Session、多 Agent 工作流的 3D 记忆宫殿原型。

它把“回到一个会话”这件事从列表、卡片和日志里拿出来，变成一个可以被空间快速识别的动作：项目是房间，Session 是房间里的工作台，Agent / IDE 是工作台的属性。用户回到工作时，不只是阅读摘要，而是先回到一个熟悉的位置，判断自己进入了哪个项目、哪个会话，以及这个项目上一次停在什么阶段。

## 记忆宫殿是什么

记忆宫殿是一种古老的空间记忆方法。它的核心想法很简单：人对“位置”的记忆通常比对抽象文字更稳定，所以可以把需要记住的信息放进一个熟悉的空间里，例如一栋房子、一条街、一个房间或一条经常走的路线。

当你想起这些信息时，不是从一张列表里逐条搜索，而是在脑子里“走回”那个空间：门口放着一件事，桌上放着一个名字，楼梯旁边放着一个线索。很多人是从福尔摩斯、记忆术或侦探作品里第一次听说这个概念，但它背后的机制并不神秘：它利用的是人类天然擅长的场景识别、路径感和位置联想。

Hearth 借用了这个思路，但重点不是让用户背下项目之间的关系，而是帮助用户在回到 Agent 会话时更快恢复上下文。项目、Session、Agent、IDE 和待处理状态不再只是平铺的信息条目，而是被安排进一个稳定的 3D 空间。这样用户可以通过房间位置、房间内工作台的位置、灯光和装饰变化，快速判断自己回到了哪一个会话、哪一个项目，以及这个项目上一次大概处于什么阶段。

![Hearth 3D memory palace](public/assets/readme-hearth.jpg)

![Hearth collapsed floating icon](public/assets/readme-hearth-collapsed.jpg)

## 核心想法

当一个人同时使用多个 IDE、多个 AI Agent、多个项目和多个会话时，真正困难的不是“有没有信息”，而是“我怎么快速知道自己回到了哪里”。

传统的多 Agent 信息汇总看板通常依赖结构化语言来恢复记忆：它会告诉你上一次会话叫什么、新弹出的 AI 回复是什么、这个 Session 的标题是什么。这样的信息是准确的，但阅读本身有时间成本。项目一多，用户也很难只靠几行短文本立刻想起这个项目到底是什么、上一次做到哪里。

Hearth 的目标是让工作上下文拥有稳定的空间形态，让用户先通过空间位置和视觉状态完成一次快速定位：

- 我回到了哪个项目，可以从房间的位置、颜色和墙面标记看出来。
- 我回到了哪个 Session，可以从房间里的工作台位置看出来。
- 这个项目上一次处于什么阶段，可以从灯光、工作台状态和房间内变化看出来。
- 哪些项目需要回看，可以从亮灯的房间和整体建筑状态看出来。

## 概念模型

### Project = 房间

每个项目对应一间房间。房间不是信息卡片的 3D 化，而是一个可记忆的位置：它在建筑中的层级、颜色、窗户和墙面标记共同构成项目识别。

### Session = 工作台

一个项目可以有多个 Session，因此房间里可以有多张工作台。每张工作台代表一个正在运行、已回复或暂时空闲的会话。

### Agent = 属性

Agent 和 IDE 不是最外层叙事。它们更像工作台和房间的属性，通过颜色、灯光、装饰件和状态细节表达。

### Attention = 灯光

需要用户回看的项目会亮灯。Hearth 不试图把所有信息都展示出来，而是先回答一个更重要的问题：哪里需要我重新进入？

## 交互形态

Hearth 有两个主要状态：

- **收起态**：一个可以悬浮在桌面或 IDE 上方的小建筑图标，低打扰地提示项目状态。
- **展开态**：一个透明悬浮的 3D 剖面沙盘，展示项目房间、Session 工作台、梯子、栏杆、花坛和项目首字母标识。

它不是第一人称漫游，也不是完整 3D 游戏。它更接近一个可以快速扫视的等距建筑模型：打开快、识别快、切回上下文快。

## 当前 MVP

已实现：

- 3D 建筑剖面
- 收起态小建筑 icon
- 透明 Electron 桌面悬浮窗
- 项目房间与 Session 工作台
- 基于 Session 状态的房间亮灯
- 工作台台灯和屏幕状态
- 房间墙面项目首字母
- 标准化结构件：柱子、梯子、栏杆、花坛
- 明亮等距插画风配色
- Hover / 聚焦时的项目信息面板

暂不包含：

- 真实 Agent / IDE Webhook
- 自动上下文总结
- 持久化用户数据
- 大量项目的自动空间排布
- 完整资产建模流程
- 打包发布流程

## 本地运行

安装依赖：

```bash
npm install
```

浏览器预览：

```bash
npm run dev
```

默认地址：

```text
http://127.0.0.1:5173/
```

桌面悬浮窗预览：

```bash
npm run desktop:dev
```

如果 Vite dev server 已经在运行，也可以只启动 Electron：

```bash
npx electron electron/main.cjs
```

构建并用 Electron 打开：

```bash
npm run desktop
```

## 技术栈

- Vite
- React
- TypeScript
- Three.js
- React Three Fiber
- Drei
- Tailwind CSS
- Electron

## 项目结构

```text
electron/
├── main.cjs              # 透明悬浮窗口、窗口模式切换、拖拽 IPC
└── preload.cjs           # 暴露桌面窗口 API

src/
├── App.tsx
├── components/hearth3d/
│   ├── Hearth3D.tsx              # 3D 交互入口与视图状态
│   ├── BuildingModel.tsx         # 展开态建筑剖面
│   ├── CollapsedBuildingIcon.tsx # 收起态小建筑
│   ├── ProjectRoom3D.tsx         # 项目房间和墙面首字母
│   ├── SessionDesk3D.tsx         # Session 工作台
│   ├── DeskLamp.tsx              # 工作台灯
│   ├── WindowLight.tsx           # 外立面窗灯
│   ├── CameraRig.tsx             # 相机过渡
│   ├── layout.ts                 # 房间布局
│   └── spaceTokens.ts            # 空间尺寸与颜色 token
├── data/projects.ts              # 项目、Session、Agent 假数据
└── types/hearthDesktop.d.ts      # Electron preload 类型

public/assets/readme-hearth.jpg             # README 展开态截图
public/assets/readme-hearth-collapsed.jpg   # README 收起态截图
```

## 设计原则

Hearth 不想做一个更重的任务管理器。它的重点是降低“回到上下文”的认知摩擦。

空间、颜色、灯光和重复出现的地标可以帮助用户建立记忆。当用户看见这个小建筑时，应该能很快想起来：我在哪里，我刚才在做什么，下一步该回到哪个房间。
